...
................................................................................
               ...
               ...................................................
			     ...
			     updated for HydroBase 3 Dec 2009
...
.   points to be connected in GMT. -D delimits a depth range for properties.
.   -F option specifies a minimum value of X,Y as a limit (default is -9 to avoid outputting 
.    missing values).
.
...
/*  parse the command line arguments */
   ...
   do {
     ...
     while ((status = get_station(infile, &hdr, &station)) == 0) { 
       ...
       any_scans = 0;
       if (xaddr != NULL && yaddr != NULL) {

          xprop = *xaddr;
	  yprop = *yaddr;
          for (n = 0; n < hdr.nobs; ++n) {
            scan_ok = (xprop[n] > flag) && ( yprop[n] > flag);
            if (scan_ok && dflag) {
               d = station.observ[(int)DE][n];
               scan_ok = (d >= (double)mindepth) && (d <= (double)maxdepth);
            }
            
            if (scan_ok) {
               ++any_scans;
               fprintf(outfile,str_format, xprop[n], yprop[n]);
            }              
...
void print_usage(char *program)
{
   ...
   fprintf(stderr,"\nthis utility.  Use the -M option to separate profiles with a > or other character.");
   fprintf(stderr,"\nUse the -Z option to delimit a depth range.\n");
   fprintf(stderr,"\nUsage:  %s -X<x_property> -Y<y_property> [-D<dirname>] [-E<file_extent>] [-Zmindepth/maxdepth] [-M<character>] [-O<outfile>] [-T<68|90>]", program);
